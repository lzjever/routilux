...
class WorkerCreateRequest(BaseModel):
    ...
    flow_id: str = Field(
        ...,
        description="Flow ID to instantiate as a Worker"
    )
...
class WorkerResponse(BaseModel):
    ...
    flow_id: str = Field(
        ..., 
        description="Flow this Worker is executing"
    )
    status: str = Field(
        ..., 
    ...
    jobs_processed: int = Field(
        0, 
        description="Number of jobs successfully processed"
    )
...
class WorkerListResponse(BaseModel):
    ...
    limit: int = Field(
        100,
        description="Maximum workers per page",
        ge=1,
        le=1000
    )
    offset: int = Field(
        0,
        description="Number of workers skipped",
        ge=0
    )
