...
def _load_class(class_spec: Union[str, Type]) -> Type:
    ...
    if not isinstance(class_spec, str):
        raise ValueError(f"Class specification must be a string or class object, got {type(class_spec)}")
    
    # Parse module path and class name
    parts = class_spec.split(".")
    if len(parts) < 2:
        raise ValueError(f"Invalid class path: {class_spec}. Expected format: 'module.path.ClassName'")
