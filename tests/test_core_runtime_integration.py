...
from routilux.core import Flow, Routine, Runtime, get_flow_registry
...
class TestEventRouting:
    ...
    def test_simple_event_routing(self):
        ...
        if len(received_data) == 0:
            ...
            pytest.fail(
                f"Event routing failed - no data received after {max_wait}s. "
    ...
    def test_fan_out_event_routing(self):
        ...
        if len(results) < 2:
            pytest.fail(
                f"Fan-out routing failed - only {len(results)} targets received data after {max_wait}s"
            )
...
class TestJobContextPropagation:
    ...
    def test_job_context_available_in_routines(self):
        ...
        if len(job_ids) < 2:
            pytest.fail(
                f"Job context propagation failed - only {len(job_ids)} routines saw job_id after {max_wait}s"
            )
