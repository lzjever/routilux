...
def validate_flow(flow: "Flow") -> List[str]:
    ...
    if unconnected_events:
        ...
        issues.append(
            f"Warning: Unconnected events (data will be lost): {', '.join(event_strs)}"
        )
    ...
    for connection in flow.connections:
        ...
        if source_routine_id is None:
            ...
            issues.append(
                f"Connection references source routine not in flow: {source_routine_id}"
            )
        ...
        if target_routine_id is None:
            ...
            issues.append(
                f"Connection references target routine not in flow: {target_routine_id}"
            )
